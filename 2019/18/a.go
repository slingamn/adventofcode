...
type Progress struct {
	position Coordinate // where we are now (position of last key collected)
...
func (t *Tour) Visit(position Coordinate, door byte, cost int) (result Tour) {
	result = *t
...
func (g Grid) ShortestTour(start Coordinate) (result Tour) {
	numKeys := g.Length()
	...
	for len(queue) > 0 {
		...
		distances := curGrid.ComputeDistances(tour.position)
		for coord, distance := range distances {
			door := curGrid.Get(coord)
			nextTour := tour.Visit(coord, door, distance)
...
func readStdin() (result Grid, start Coordinate) {
	isWall := func(line []byte) bool {
		for _, c := range line {
			if c != '#' {
				return false
			}
