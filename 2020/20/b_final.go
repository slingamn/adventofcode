...
func recursiveBacktrack(allTiles map[int]Tile, result [][]int, rotations [][]uint8, tileArrangement [][]Tile, availableTiles []int, y, x int) (success bool) {
	nextY := y
	...
	for i, tileId := range availableTiles {
		if tileId == 0 {
			continue
		}
		tilesRemaining = true
		...
		for rotIdx, r := range tile.DihedralFour() {
			if compatible(tileArrangement, y, x, r) {
				...
				if recursiveBacktrack(allTiles, result, rotations, tileArrangement, availableTiles, nextY, nextX) {
					return true
				}
			}
		}

		availableTiles[i] = tileId
	}

	if !tilesRemaining {
		// base case: all tiles have been successfully placed
		return true
	}
...
func arrange(tiles map[int]Tile) (result [][]int, rotations [][]uint8) {
	...
	availableTiles := make([]int, 0, len(tiles))
	for tileId := range tiles {
		availableTiles = append(availableTiles, tileId)
	}

	success := recursiveBacktrack(tiles, result, rotations, ta, availableTiles, 0, 0)
	if !success {
		panic("fail")
	}
	return
